...
class Client {
    ...
    private createRequest<T>(type: MessageTypes, ...params: any[]): Promise<T> {
        const promise = new Promise<T>((resolve, reject) => {
            try {
                this.socket.emit(type, this.id, ...params, (result: T) => {
                    resolve(result)
                })
            } catch (error) {
    ...
    public subscribeToRoom(
        tickCallback: (hazard: IHazard, entities: [string, IEntity][]) => void,
        deathCallback: (username: string) => void
    ): Promise<string> {
