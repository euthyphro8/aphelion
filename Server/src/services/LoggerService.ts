...
class LoggerService {
    ...
    constructor(logName: string, filename: string, maxFiles: number, maxSize: number) {
        console.log('\n-- LOGGER INITIALIZED --');
        const customLevels = {
            levels: {   // Levels in accordance with rfc5424 https://tools.ietf.org/html/rfc5424
        ...
        const customPrint = Winston.format.printf(({label, level, message, timestamp}) => {
            let msg = level;
